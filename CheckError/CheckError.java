...
public class CheckError {
    private static String dir = "C:\\Users\\jun\\Desktop\\problem_1\\"; // 需要验证的程序目录
    private static int inputNum = 10; // 使用10组样例测试代码
    ...
    public static void main(final String[] args) throws IOException, InterruptedException {
        ...
        if (files == null) {
            System.out.println("compiling failed, no any file to be compiling!");
            return;
        }
        ...
        for (int n = 0; n < inputNum; ++n) { // 使用多组输入判断程序和正确程序的输出是否相同
            final String input = constructInput(n); // 构造输入
            System.out.println("=====================================================================================");
            ...
            System.out.println("correct result: " + cpResult);

            // 运行其他程序，将结果与正确程序比较，看看是否正确
            int errorCount = 0;
            assert files != null;
            for (final File file : files) {
                if (file.getName().endsWith(".exe") && !correctProgram.equals(file.getName())) {
                    final String result = runApplication(file.getPath(), input);
                    assert cpResult != null;
                    if (!cpResult.equals(result)) {
                        errorCount++;
                        System.out.println(errorCount + "\t" + "error in: " + file.getName() + " || result: " + result);
                    }
                }
            }
        }
