...
if __name__ == '__main__':
    ...
    ipPattern = '\d+\.\d+\.\d+\.\d+'
    pattern = '(' + ipPattern + '\s' + ipPattern + '\s(' + ipPattern + '|[\uAC00-\uD7A3\u0800-\u4e00\u4e00-\u9fa5A-Za-z]+)\s' + ipPattern + '\s' + '\d+)'
    ...
    for idx,rl in enumerate(routerList):
        row = re.split(r'\s', rl[0])
        if row[2] == ipGateway:
            if routeIsNeed(row[0], row[1], njuIps) == False:
                log = execCmd("route delete " + row[0])
                if re.search('success', log) is None and re.search('成功', log) is None and re.search('完成', log) is None:
                    flag = 0
                    print('\n权限不足，无法删除路由表，请以管理员身份运行该程序！')
                    break
                else:
